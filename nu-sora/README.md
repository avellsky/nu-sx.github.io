...
| 通報・社会実装 | G-7 / PF-3 | **通報ワークフロー（各段階をクリックすると解析経過が開く）**、警戒レベルの定義、通報ログ、法的論点、連携先 |
...
### 通報ワークフローの各段階（「通報・社会実装」画面）

検出 → 多点対応 → 軌跡決定 → 落下域 → 通報 → 確認 → 対応 の 7 段階を図で示し、
各段階をクリックするとその時点の解析経過（入力・処理・出力・その時点で何が分かるか）が開く。
