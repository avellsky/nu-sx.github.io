...
NS.dracoSection = function (st, go) {
  ...
  out.push(panel('Draco スマート望遠鏡　' + st.name, {
    ...
    el('p', { style:{ margin:'0 0 12px', color:'var(--ink2)' },
      text:'全天カメラは視野 95° で空全体を見張り、ガンダム望遠鏡は主鏡 600 mm の視野 0.28° で一点を深く見る。その間を埋めるのが Draco である。'
  ...
  out.push(panel('この望遠鏡の使いみち', { note:'全天カメラ（95°）→ Draco（1.7°）→ ガンダム望遠鏡 副鏡（1.07°）→ 主鏡（0.28°）と視野を絞っていく' },
    ...
    NS.table(['観測局', '所在地', '夜空輝度', 'この局に置く理由'], NS.DRACO.at.map(function (id) {
      ...
      return [el('b', { text:x.name }), x.pref + ' ' + x.city,
        ...
        { class:'sm', html:id === 'FNB'
          ? 'ハブ局。ガンダム望遠鏡（600 / 200 mm）と同じ屋上に置き、視野の広さで役割を分ける。分光・SWIR も揃うため、一つの事象を複数の装置で同時に押さえられる'
